...
var simulator = {
    ...
    operations: {
        getRobot: function(id) {
            var rebuild = false, robot = simulator.robots[id];
            if (!robot) {
        ...
        calcNewPosition: function(robot, t) {
            ...
            distance[0] = robot.wheel.circumference * (delta[0] / 360);
            distance[1] = robot.wheel.circumference * (delta[1] / 360);
        ...
        moveTo : function(robot, wheelPos1, wheelPos2) {
            robot.newLocation = [wheelPos1, wheelPos2];
            robot.state = 'move';
        },
        move: function(robot, wheelPos1, wheelPos2) {
            robot.newLocation[0] = robot.newLocation[0] + wheelPos1;
            robot.newLocation[1] = robot.newLocation[1] + wheelPos2;
            robot.state = 'move';
        }
    }
};
    ...
    if (Linkbots) {
        (function(proxied) {
            window.Linkbots.acquire = function(v) {
                console.log('called proxied acquire');
                var robots = proxied(v);
                for (var i = 0; i < robots.length; i++) {
                    simulator.operations.getRobot(robots[i].id);
                }
                return robots;
            };
        })(Linkbots.acquire);
        ...
        for (key in simulator.robots) {
            ...
            if (robot.state !== 'stop') {
                if (robot.wheel.direction[0] === 0 && robot.wheel.direction[1] === 0) {
                    robot.state = 'stop';
                    console.log('stopping robot ' + robot.id);
                }
                draw = true;
